"""CLI commands extensions for logging and cron setup."""
...
from nanobot.utils.logging import setup_logging as _setup_logging
...
def setup_logging(verbose: bool = False) -> Path:
    ...
    Returns:
        Path to log file
    """
    return _setup_logging(verbose=verbose)
...
def setup_cron_with_agent(cron_service, bus: "MessageBus", agent: "AgentLoop"):
    """
    Setup cron service with proper callback after agent is created.
    
    Args:
        cron_service: CronService instance
        bus: Message bus
        agent: Agent loop
    """
    cron_service.on_job = create_cron_callback(bus, agent)
